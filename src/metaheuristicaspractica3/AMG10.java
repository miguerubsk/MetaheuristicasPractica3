...
public class AMG10 {
    ...
    private void PrimerMejor(){
        ...
        boolean end, mejora;
        int contIter, costeAct, costeTmp, valor;
        int numIndividuos = (poblacion.getTam()*PORCENTAJE)/100;
        int[] seleccionados = new int[numIndividuos];
        boolean[] selec = new boolean[poblacion.getTam()];
        for(int i=0; i<poblacion.getTam(); i++){
            selec[i] = false;
        }
        //Se seleccionan los individuos a los que se les aplicara la busqueda local.
        for(int i=0; i<numIndividuos; i++){
            do{
                valor = rand.nextInt(poblacion.getTam());
            }while(selec[valor]);
            seleccionados[i] = valor;
            selec[valor] = true;
        }
        //Se aplica la busqueda local a los individuos.
        for(int k=0; k<numIndividuos; k++){
            end = false;
            for(int i=0; i<nuevaGen.individuo(seleccionados[k]).getTam(); i++){
                dlb[i]=0;
            }
            contIter = 0;
            costeAct = nuevaGen.individuo(seleccionados[k]).getCoste();
            while(!end && contIter < MAX_ITERACIONES){
                for(int i=0; i<nuevaGen.individuo(seleccionados[k]).getTam(); i++){
                    if(dlb[i]==0){
                        mejora = false;
                        for(int j=0; j<nuevaGen.individuo(seleccionados[k]).getTam(); j++){
                            nuevaGen.individuo(seleccionados[k]).Intercambio(i, j);
                            costeTmp = nuevaGen.individuo(seleccionados[k]).CosteIntercambio(i, j);
                            if(costeTmp < costeAct){
                                ...
                                nuevaGen.individuo(seleccionados[k]).Intercambio(j, i);
                                nuevaGen.individuo(seleccionados[k]).CosteIntercambio(j, i);
                            }
                        }
                        if(!mejora){
                            dlb[i]=1;
